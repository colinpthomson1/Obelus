...
/**
 * Ensures Windows shims are available in %LOCALAPPDATA%\Obelus\bin.
 */
export async function ensureWinShims(): Promise<void> {
  ...
  try {
    ...
    if (!currentPath.toLowerCase().includes(tgtDir.toLowerCase())) {
      ...
      log.info(`Added ${tgtDir} to PATH for Obelus processes only`);
    } else {
      ...
      if (binDirIndex > 0) {
        ...
        log.info(`Moved ${tgtDir} to beginning of PATH for Obelus processes only`);
      }
    }
  } catch (error) {
