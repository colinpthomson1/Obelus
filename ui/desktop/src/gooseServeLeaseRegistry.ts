...
export const GOOSE_SERVE_EXITED_USER_MESSAGE =
  "This window's Obelus backend stopped. Close this window and open a new chat to start a new backend. If this keeps happening, restart Obelus.";
...
export class GooseServeLeaseRegistry {
  ...
  create(result: GooseServeResult, secretKey: string): GooseServeLease {
    ...
    const markExited = ({
      ...
      if (logUnexpected && firstExit && !lease.cleanedUp) {
        this.logger.error('Obelus backend exited unexpectedly', {
          code: lease.exitCode,
  ...
  async cleanupLease(lease: GooseServeLease) {
    ...
    try {
      ...
      this.logger.error('Failed to clean up the Obelus backend:', error);
    }
  }
