document.addEventListener("DOMContentLoaded", () => {
  ...
  isBuiltinCheckbox.addEventListener("change", () => {
    nonBuiltinFields.style.display = isBuiltinCheckbox.checked
      ? "none"
      : "block";
  });
  ...
  function generateInstallLink(server) {
    if (server.is_builtin) {
      ...
      return `obelus://extension?${queryParams}`;
    }
  ...
  const sampleJson = {
    ...
    description: "An example Obelus extension",
    command: "node ./path/to/extension.js",
    environmentVariables: [
      {
        name: "API_KEY",
